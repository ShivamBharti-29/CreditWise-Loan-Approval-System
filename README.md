# Loan Approval Prediction System 🏦

A high-performance machine learning application that predicts loan eligibility based on 19 financial and personal factors.

## 🚀 Features
- **Dual Model Support**: Toggle between a fast Logistic Regression and a high-accuracy Random Forest.
- **Automated Pipeline**: Uses Scikit-Learn `Pipeline` for seamless data preprocessing.
- **Interactive Web UI**: Built with Streamlit for real-time predictions.

## 📊 Model Performance
- **Random Forest**: ~95.3% Accuracy (Primary Model)
- **Logistic Regression**: ~89.4% Accuracy (Baseline)

## 🛠️ Tech Stack
- **Language**: Python
- **Libraries**: Scikit-Learn, Pandas, NumPy, Joblib
- **Deployment**: Streamlit
